export function dcTrialslist() {
  ...
  var sheetDatacenter = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
    "Datacenter"
  ...
  var sheetMembers = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
    "Members"
  ) as GoogleAppsScript.Spreadsheet.Sheet;
  var items2 = sheetMembers.getDataRange().getValues();
  var mem1 = 2; //Group
  var mem2 = 3; //Sortoder
  var targetSheetFull = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
    "DCtrialslistFull"
  ) as GoogleAppsScript.Spreadsheet.Sheet;
  const oldtargetSheetName = "DCtrialslist";
  var oldtargetSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
    oldtargetSheetName
  ) as GoogleAppsScript.Spreadsheet.Sheet;
  var study = [];
  var number = 1;
  ...
  if (ReferenceDate.isBlank()) {
    Browser.msgBox(
      "A1をダブルクリックして基準日を選択してから、再度「DCtrialslist作成」を実行してください。\\n\\nスクリプトを終了します。"
    );
    return;
  } else {
    var RefDate = Utilities.formatDate(
      ReferenceDate.getValue(),
      "JST",
      "yyyy/MM/dd"
    );
    var msg = Browser.msgBox(
      "『" +
        RefDate +
        "』を基準日として実行します。よろしければOKを選択してください。\\n\\n日付を変更する場合キャンセルを選択して、A1をダブルクリックして正しい日付を選択してください。\\n\\n誤って起動した場合は、キャンセルを選択してください。",
  ...
  for (var i = 1; i < items.length; i++) {
    if (
      !items[i][DC11].includes("NoSupport") &&
      !items[i][DC11].includes("Uncertain") &&
      items[i][DC1] != ""
    ) {
      //掲示対象判定
      const del =
        items[i][DC11].includes("Discontinued") ||
        items[i][DC11].includes("Completed")
          ? "Delete"
          : "";
      //status
      switch (true) {
      ...
      switch (true) {
        case items[i][DC13] == "医師主導治験" ||
          (items[i][DC13] == "国際共同試験" && items[i][DC4] == "Ptosh"):
          star = "○";
          break;
        default:
          star = "";
  ...
  targetSheetFull
    .getRange(2, 1, countbefore, 1)
    .setValue("開\n始\n前\n" + countbefore + "\n件");

  const after = targetSheetFull.getRange(countbefore + 2, 1, countafter, 1);
  after.merge().setHorizontalAlignment("center").setVerticalAlignment("middle");
  targetSheetFull
    .getRange(countbefore + 2, 1, countafter, 1)
    .setValue("開\n始\n後\n" + countafter + "\n件");

  const registry = targetSheetFull.getRange(
    countbefore + countafter + 2,
    1,
    countregistry,
    1
  );
  ...
  targetSheetFull
    .getRange(countbefore + countafter + 2, 1, countregistry, 1)
    .setValue("疫学研究/疾患登録" + countregistry + "件");
  ...
  //罫線
  targetSheetFull
    .getRange(1, 1, countbefore + countafter + countregistry + 1, 8)
    .setBorder(
      true,
      true,
      true,
  ...
  targetSheetFull
    .getRange(1, 9, countbefore + countafter + countregistry + 1, 2)
    .setBorder(
      true,
      true,
      true,
  ...
  targetSheetFull
    .getRange(1, 11, countbefore + countafter + countregistry + 1, 4)
    .setBorder(
      true,
      true,
      true,
  ...
  targetSheetFull
    .getRange(countbefore + 2, 1, countafter, 14)
    .setBorder(
      true,
      true,
      true,
  ...
  targetSheetFull
    .getRange(countbefore + countafter + 2, 1, countregistry, 14)
    .setBorder(
      true,
      true,
      true,
  ...
  targetSheetFull
    .getRange(1, 1, countbefore + countafter + countregistry + 1, 1)
    .setBorder(
      true,
      true,
      true,
  ...
  //件数入力
  newtargetSheet
    .getRange(2, 1, listcountbefore, 1)
    .setValue("開\n始\n前\n" + listcountbefore + "\n件");
  newtargetSheet
    .getRange(listcountbefore + 2, 1, listcountafter, 1)
    .setValue("開\n始\n後\n" + listcountafter + "\n件");
  newtargetSheet
    .getRange(listcountbefore + listcountafter + 2, 1, listcountregistry, 1)
    .setValue("疫学研究/疾患登録" + listcountregistry + "件");

  //罫線
  newtargetSheet
    .getRange(1, 1, listcountbefore + listcountafter + listcountregistry + 1, 8)
    .setBorder(
      true,
      true,
      true,
  ...
  newtargetSheet
    .getRange(1, 9, listcountbefore + listcountafter + listcountregistry + 1, 2)
    .setBorder(
      true,
      true,
      true,
  ...
  newtargetSheet
    .getRange(2, 1, listcountbefore, 10)
    .setBorder(
      true,
      true,
      true,
  ...
  newtargetSheet
    .getRange(listcountbefore + 2, 1, listcountafter, 10)
    .setBorder(
      true,
      true,
      true,
  ...
  newtargetSheet
    .getRange(listcountbefore + listcountafter + 2, 1, listcountregistry, 10)
    .setBorder(
      true,
      true,
      true,
  ...
  newtargetSheet
    .getRange(1, 1, listcountbefore + listcountafter + listcountregistry + 1, 1)
    .setBorder(
      true,
      true,
      true,
